...
Сервис анкоринга делает приложения на базе Exonum более безопасными и отказоустойчивыми. 
Этот сервис периодически публикует хеш
...
непотраченными выходами транзакции для пополнения баланса ([UTXO пополнения баланса](#funding-utxo)).
...
валидаторов  (`N <= 20` из-за ограничений в сети Биткоин), а `M` &mdash; необходимое
...
Каждый валидатор обновляет свою LECT по [специальному расписанию](#lect-updating-interval).
Чтобы получить новую LECT, валидатор использует API [узла Биткоина](#bitcoind-node).
...
цепочка оборвалась (как описано в разделе [Восстановление предыдущей цепочки](#recovering-broken-anchoring)).
