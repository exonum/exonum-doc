...
[Алгоритм консенсуса][wiki:consensus] – это процесс получения
...
2. Валидаторы могут голосовать за это предложение, транслируя сообщение типа *prevote*.
   Это сообщение означает, что валидатор смог обработать предложение и у валидатора
   есть все транзакции указанные в предложении.
...
!!! note "Пример"
    ...
    Несмотря на это, валидаторы B и C могут запросить сообщения друг друга у валидатора A.
...
Реализовны запросы на все консенсусные сообщения: proposal, prevote и precommit.
...
Сообщение `Status` - это информационное сообщение о текущей высоте блокчейна. Оно
